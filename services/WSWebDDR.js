...
class WebDDR {
  ...
  initWebSocket(pWebSocketConnectedCallback) {
    console.log('WebDDR Inside - InitWebSocket');
    try {
      ...
      console.log(
        'WebDDR: Exception thrown during the initialization of the DDR webSocket: ' +
          exception
      );
    }
  }
   ...
   */
  webSocketClosed(evt) {
    this.context.connectionState = false;
    console.log('Connection Status Error:', this.context.connectionState);
    console.log(
      'WebDDR: DDR webSocket Client disconnected from the address: ' +
        this.context.wsUri
    );
    // The server is offline try to reconnect in 2 seconds.
    this.webSocket = undefined;
    setTimeout(() => {
      this.context.initWebSocket(this.context.webSocketConnectedCallback);
    }, 5000);
  }
  ...
  webSocketError(evt) {
    this.context.connectionState = false;
    console.log('Connection Status Error:', this.context.connectionState);
    console.log("WebDDR: DDR webSocket Error: evt='" + evt.message + "'");
    if (
      this.context.webSocketErrorCallback !== undefined &&
      this.context.isFunction(this.context.webSocketErrorCallback)
    ) {
  ...
  onwebSocketMessageReceived(pParams) {
    //console.log('pParams from webSocketMsgRec', pParams);
    let lJsonResponse = JSON.parse(pParams.data);
    //console.log('***lJsonResponse*** \n', lJsonResponse);
    if (lJsonResponse.hasOwnProperty('getValueResponse')) {
      ...
      this.context.processGetVariableCountResponse(
        lJsonResponse['getVariableCountResponse']
      );
    } else if (
      lJsonResponse.hasOwnProperty('onVariableValueChangedNotification')
    ) {
      this.context.processOnVariableValueChangedNotification(
        lJsonResponse['onVariableValueChangedNotification']
      );
    } else if (lJsonResponse.hasOwnProperty('getVariableDefinitionsResponse')) {
      this.context.getVariableDefinitionsCallback(
        lJsonResponse['getVariableDefinitionsResponse']
      );
    }
  }
  ...
  setVariableValue(pVariableName, pVariableValue) {
    console.log("setVariableValue ----- ", pVariableName, pVariableValue);
    if (typeof pVariableName !== 'string') {
   ...
   */
  subscribeVariablesValue(
    pVariableNameList,
    pCallback,
    pUseTheSameCallbackForAllVariables
  ) {
    ...
    if (this.isFunction(pCallback)) {
      //console.log("pCallback", pCallback);
    ...
    for (let index = 0; index < pVariableNameList.length; ++index) {
      ...
      if (typeof lVariableName !== 'string') {
        throw 'WebDDR: The values inside pVariableNameList must be strings';
      }
  ...
  sendGetValueRequest(pVariableName) {
    // console.log('sendGetValueRequest --', pVariableName);
    let lSuccess = false;
  ...
  sendSetVariableValueRequest(pVariableName, pVariableValue) {
    ...
    if (
      ...
      console.log('sendSetVariableValueRequest lVariable ******', params);
      this.webSocket.send(JSON.stringify(params));
      lSuccess = true;
    }
  ...
  sendSubscribeVariablesRequest(pVariableNameList) {
    ...
    if (
      ...
      //console.log('sendSubscribeVariablesRequest params', params);
      this.webSocket.send(JSON.stringify(params));
      lSuccess = true;
    }
  ...
  processGetValueResponse(pVariable) {
    //console.log('processGetValueResponse ----', pVariable);
    let lVariable = [];
    lVariable.name = pVariable['variableName'];
    lVariable.value = pVariable['variableValue'];

    //console.log('lVariable', lVariable);
    ...
    while (this.getVariableValueCallbackArray[lVariable.name].length !== 0) {
      ...
      let lCallBack =
        this.getVariableValueCallbackArray[lVariable.name].shift();
      lCallBack(lVariable);
    }
  }
  ...
  processOnVariableValueChangedNotification(pVariable) {
    ...
    if (this.onVariableValueChangedNotificationCallback !== undefined) {
      ...
      for (
        let j = 0;
        j <
        this.onVariableValueChangedNotificationCallbackArray[lVariable.name]
          .length;
        ++j
      ) {
        let lCallBack =
...
//export default WebDDR;
module.exports = WebDDR;
